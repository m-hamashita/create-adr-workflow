...
repository_id=$(gh api graphql --jq '.data.repository.id' -f query='query {
      repository(owner: "'$REPOSITORY_OWNER'", name: "'$REPOSITORY_NAME'") {
        id
      }
    }'
)
category_id=$(gh api graphql --jq '.data.repository.discussionCategories.nodes[] | select(.name=="ADR") | .id' -f query='
        {
          repository(owner: "'$REPOSITORY_OWNER'", name: "'$REPOSITORY_NAME'") {
...
adr_num=$(gh api graphql --jq '.data.repository.discussions.nodes[].title' -f query='query {
     repository(owner: "'$REPOSITORY_OWNER'", name: "'$REPOSITORY_NAME'") {
